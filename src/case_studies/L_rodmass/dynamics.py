...
class RodMassDynamics(DynamicsBase):
    ...
    def f(self, x, u):
        ...
        # Equation of motion:
        # m*l^2*thetaddot + b*thetadot + k1*theta + k2*theta^3 + m*g*l*sin(theta) = tau

        thetaddot = (1 / (self.m * self.ell**2)) * (
            tau
            - self.b * thetadot
            - self.k1 * theta
            - self.k2 * theta**3
            - self.m * self.g * self.ell * np.sin(theta)
        )
